            ...
            match msg_in.union {
                ...
                Some(rendezvous_message::Union::RegisterPk(rk)) => {
                    ...
                    if id.len() < 6 {
                        ...
                        // Use TOO_FREQUENT (not UUID_MISMATCH) so the client keeps its
                        // stable id — UUID_MISMATCH makes it regenerate its id and cycle.
                        // The client's RegisterPkResponse handler has no arm for this, so
                        // it simply keeps the id and retries slowly (stays not-registered).
